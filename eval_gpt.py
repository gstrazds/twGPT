...
def play_game(gamename, pl_module, tokenizer, gamedir=TW_TRAINING_DIR, cmds=None, max_steps=45, using_internal_names=False, step_infos=None):
    ...
    _predicted_cmd = predict_cmd(pl_module, tokenizer, pthru_so_far, failed_cmds=None)
    if using_internal_names:
        predicted_cmd = subst_names(_predicted_cmd, map_names2ids, ids2names=True)
    else:
        predicted_cmd = _predicted_cmd
    ...
    while not all(_dones) and num_steps < max_steps:
        ...
        if cmd_was_ok:
            ...
            if _predicted_cmd in attempted_cmds or len(attempted_cmds) > 4:  # our previous attempts to randomize failed to find any other option
                print("Stop trying to find alternatives:", _predicted_cmd, " has already been tried:", attempted_cmds)
                num_steps += 1
            attempted_cmds.append(_predicted_cmd)

        if not _dones[0]:
            _predicted_cmd = predict_cmd(pl_module, tokenizer, pthru_so_far, attempted_cmds)
            if using_internal_names:
                predicted_cmd = subst_names(_predicted_cmd, map_names2ids, ids2names=True)
            else:
                predicted_cmd = _predicted_cmd
...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.eval.play_games:
        ...
        for idx in range(len(eval_ds)):
            ...
            if num_steps < cfg.eval.max_steps:  # n_steps is not None and n_steps < 45:
                maybe_ok += 1
            if won:
                num_successful += 1
