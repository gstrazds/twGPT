...
class FeedbackAttention(Module):
    ...
    def __init__(self, n_heads: int, d_model: int, dropout_prob: float = 0.1, *,
        ...
        * `is_kv_precomputed` is whether key, value tensors are already calculated
        ...
        self.key_pos_embeddings = nn.Parameter(torch.zeros((self.P, n_heads, self.d_k)), requires_grad=True)
        # Relative positional embedding bias for key relative to the query.
        self.key_pos_bias = nn.Parameter(torch.zeros((self.P, n_heads)), requires_grad=True)
        # Positional embeddings for the query is independent of the position of the query
        self.query_pos_bias = nn.Parameter(torch.zeros((n_heads, self.d_k)), requires_grad=True)
...
class AutoregressiveModel(Module):
    ...
    def forward(self, x: torch.Tensor):
        # Embed the tokens
        x = self.src_embed(x)
        # Run the input through the the transformer
...
def feedback_transformer(
    ...
    return AutoregressiveModel(
        n_vocab, d_model,
        FeedbackTransformer(
            FeedbackTransformerLayer(d_model=d_model,
                                     attn=FeedbackAttention(n_heads, d_model, dropout),
                                     feed_forward=FeedForward(d_model, d_ff, dropout),
                                     dropout_prob=dropout),
            n_layers))  #.to(c.device)


def feedback_transformer_kv(
    d_model: int = 512,
    n_heads: int = 8,
    dropout: float = 0.0,
    d_ff: int = 2048,
